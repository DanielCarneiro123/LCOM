...
int rtc_set_binary() {
    uint8_t status;
    if (rtc_command(REGISTER_B, &status)) return 1;
...
int rtc_read(uint8_t register, uint8_t *status) {
    ...
    if (sys_outb(REGISTER_INPUT, register)) return 1;
	if (util_sys_inb(REGISTER_OUTPUT, status)) return 1;
    return 0;
}

int rtc_write(uint8_t register, uint8_t data) {
    if (sys_outb(REGISTER_INPUT, register) != 0) return 1;
...
int rtc_is_updating() {
    uint8_t status;
    if (rtc_read(REGISTER_UPDATING, &status)) return 1;
